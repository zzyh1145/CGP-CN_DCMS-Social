...
// 从数据库中获取群聊信息
$chat = dbassoc(dbquery("SELECT * FROM `privat_room` WHERE `id` = '".intval($_GET['id'])."' LIMIT 1"));
...
if (dbresult(dbquery("SELECT COUNT(*) FROM `privat_room` WHERE `id` = '".intval($_GET['id'])."' LIMIT 1",$db), 0) == 0) {header("Location: index.php?".SID);exit;}
$privat = dbassoc(dbquery("SELECT * FROM `privat_room` WHERE `id` = '".intval($_GET['id'])."' LIMIT 1"));
...
if (isset($_POST['name']) && isset($_POST['password']) && isset($_POST['ok']))
{
    ...
    if (!isset($err)) { // 如果没有错误，更新数据库
        dbquery("UPDATE `privat_room` SET `name` = '$name', `password` = '$password' WHERE `id` = '$privat[id]' LIMIT 1");
...
echo "<br />"; 
echo "<input value=\"保存\" type=\"submit\" name=\"ok\"/>\n";
echo "</form>";
